...
## 3. 소제목 보고 후 (1~6번 순차 사진 수집)

주제·소제목이 보고된 뒤, **1번부터 6번까지 순서대로** 봇이 "N번 글 사진 보내주세요 (최대 3장)"라고 요청합니다. **메시지를 보낼 때까지 대기**하며(타임아웃 없음), 업무 중에도 여유 있게 보낼 수 있습니다.

| 동작 / 명령 | 설명 |
|-------------|------|
| **사진 전송** | 해당 번호 글에 넣을 사진을 보냄. **글당 최대 3장**까지 가능 (1장만 보내도 됨) |
| **다음**, **스킵**, **skip** | 해당 번호 수집 종료 후 다음 번호로 진행. 사진 없이 건너뛸 때도 사용 |

- 소제목/스케줄의 **1번~6번**은 모두 **같은 순서**(발행 시각 순)라서, "1번"이라고 요청되는 글과 실제 1번 글이 일치합니다.
...
3. 초안 생성 → **주제·소제목** 보고 (스케줄 순서와 동일)  
4. **1~6번 순차 사진 수집** (글당 최대 3장, 다음/스킵 입력 시 다음 번호. 메시지 올 때까지 대기)  
5. **발행 스케줄** 보고 → **11:00~22:00** KST 사이에 6편 자동 발행  
